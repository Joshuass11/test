...
#n_heads = 8，head_dims = 32，d_model=两个相乘，
        ...
        if scale:
            ...
            self.scale = 1
#def forward(self, x, mask):：定义了MultiHeadAttn类的前向传播方法。
    def forward(self, x, mask):
        ...
        x = self.qkv_linear(x)
        #将输出张量沿着指定维度（dim=-1，即最后一个维度）分成三份，分别得到 Query（q）、Key（k）和 Value（v）
...
#通过点积计算 Query 和 Key 得到注意力矩阵 attn，大小为 batch_size x n_head x max_len x max_len。
...
class MultiheadPosAttn(nn.Module):
    def __init__(self, d_model, n_head, dropout=0.1, scale=False):
        """
#n_heads = 8，head_dims = 32，d_model=两个相乘，
        :param d_model:
    ...
    def forward(self, x):
        ...
        :return:
        """
        batch_size, max_len, embedding_layers,_= x.size()
        x = self.qkv_linear(x)
        # 将输出张量沿着指定维度（dim=-1，即最后一个维度）分成三份，分别得到 Query（q）、Key（k）和 Value（v）
        q, k, v = torch.chunk(x, 3, dim=-1)
        q = q.reshape(batch_size, max_len, self.n_head, -1).transpose(1, 2)
        k = k.reshape(batch_size, max_len, self.n_head, -1).permute(0, 2, 3, 1)
        v = v.reshape(batch_size, max_len, self.n_head, -1).transpose(1, 2)
        #q = q.view(batch_size, max_len, self.n_head, -1).transpose(1, 2)
        #k = k.view(batch_size, max_len, self.n_head, -1).permute(0, 2, 3, 1)
        #v = v.view(batch_size, max_len, self.n_head, -1).transpose(1, 2)
        # 通过点积计算 Query 和 Key 得到注意力矩阵 attn，大小为 batch_size x n_head x max_len x max_len。
        ...
        attn_score = F.softmax(attn, dim=-1)  # batch_size x n_head x max_len x max_len
        #后期对attn_score进行处理
        attn = self.dropout_layer(attn_score)
        v = torch.matmul(attn, v)  # batch_size x n_head x max_len x d_model//n_head
        v = v.transpose(1, 2).reshape(batch_size, max_len,embedding_layers, -1)
        #v = self.fc(v)

        return v

class TransformerLayer(nn.Module):
    def __init__(self, d_model, self_attn, feedforward_dim, after_norm, dropout,n_head):
        """
        ...
        self.ffn = nn.Sequential(nn.Linear(d_model*n_head, feedforward_dim),
                                 nn.ReLU(),
                                 nn.Dropout(dropout),
...
class TransformerEncoder(nn.Module):
    def __init__(self, num_layers, d_model, n_head, feedforward_dim, dropout, after_norm=True, attn_type='naive',
                 scale=False, attn_dropout=0, pos_embed=None):
        #以下两行修改，并且这里没有改成multiquery
        super().__init__()
...
#adatrans相对位置注意力机制
        if attn_type == 'transformer':
            self_attn = MultiHeadAttn(d_model, n_head, attn_dropout, scale=scale)
        elif attn_type == 'adatrans':
            self_attn = RelativeMultiHeadAttn(d_model, n_head, attn_dropout, scale=scale)

        self.layers = nn.ModuleList([TransformerLayer(d_model, deepcopy(self_attn), feedforward_dim, after_norm, dropout,n_head)
