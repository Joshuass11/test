...
class RelativeMultiHeadAttn(nn.Module):
    def __init__(self, d_model, n_head, dropout, r_w_bias=None, r_r_bias=None, scale=False):
        ...
        self.proj = nn.Linear(d_model * n_head,
                              d_model, bias=False)
        self.dropout_layer = nn.Dropout(dropout)
        # 这里有修改
        self.pos_embed = RelativeSinusoidalPositionalEmbedding(d_model, 0, 1200)
        ...
        if r_r_bias is None or r_w_bias is None:  # Biases are not shared
            # 这里也有修改,把n-head改成1
            self.r_r_bias = nn.Parameter(nn.init.xavier_normal_(torch.zeros(1, d_model)))
            self.r_w_bias = nn.Parameter(nn.init.xavier_normal_(torch.zeros(1, d_model)))
        else:
            self.r_r_bias = r_r_bias  # r_r_bias就是v
            self.r_w_bias = r_w_bias  # r_w_bias就是u

    def _forward(self, q, k, v, batch_size, max_len, d_model, mask):
        pos_embed = self.pos_embed(mask)  # l x head_dim
        #以下把n-head改成1
        q = q.view(batch_size, max_len, 1, -1).transpose(1, 2)
        rw_head_q = q + self.r_r_bias[:, None]
        # Einstein Summation 表示法计算 Query 和 Key 之间的乘积，得到注意力分数矩阵。这个操作可以同时考虑绝对位置和相对位置的信息。
        ...
        z1 = torch.matmul(attn, v).transpose(1, 2).reshape(batch_size, max_len, d_model)  # b x n x l x d
    ...
    def forward(self, x, mask):
        ...
        v = self.key_proj(x)
        # 这里把n-head改成1
        k = k.view(batch_size, max_len, 1, -1).transpose(1, 2)
        v = v.view(batch_size, max_len, 1, -1).transpose(1, 2)  # b x n x l x d
        # 这里要修改
        z2 = torch.empty(0)
        for query in self.querys:
            chars_output = self._forward(query(x), k, v, batch_size, max_len, d_model, mask)
            z2 = torch.cat((z2, chars_output), dim=2)
        #z2 = self.proj(z2)
        return z2
